...
// g++ -Wall -g -lpthread -std=c++11 Test_harness.cc
#include <iostream>
#include <iomanip>
#include <cstdlib>
#include <fstream>
...
public:
    ...
    vector<vector<int> > checklife(vector<vector<int> > &board, int start, int end);                        // Check the type of cell
...
vector<vector<int> > GameOfLife::SimulateLife(vector<vector<int> > &board, int life_cycles){
    ...
    for(int i = 0; i < life_cycles; i++){
        ...
        // bstatus = f.get();
            future<vector<vector<int> > > a1 = async(launch::async|launch::deferred, &GameOfLife::checklife, &g, ref(solution), 0, 10);
            future<vector<vector<int> > > a2 = async(launch::async|launch::deferred, &GameOfLife::checklife, &g, ref(solution), 10, 20);
 ...
 */ 
vector<vector<int> > GameOfLife::checklife(vector<vector<int> > &board, int start, int end){
    vector<vector<int> > solution = board;    
