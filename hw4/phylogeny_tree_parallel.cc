...
// bcast, derived from slide notes 13 and the 3 steps
...
int main(int argc, char *argv[]) {
    ...
    // Required MPI inits
    MPI_Init(NULL, NULL);
    MPI_Comm_size(MPI_COMM_WORLD, &process_num); // n
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);        // k
  ...
  if(rank == 0){
    ...
    while (genome_tree.size() > 1) {
      ...
      //cout << "This is the process num: " << process_num << " This is rank: " << rank << endl;

      // root, gather all the input (answers)
      if(process_num == 0){
        int buff[2];
        for(int i = 1; i < process_num; i++){
            MPI_Recv(buff, 2, MPI_INT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            // check for min index after this to break ties
        }
      }
      else{ // all other processes start to work on their genomes, mostly what was already given 
        int buff[2];
        string best;
        bool start = true;
        for (int i=rank;i<genome_tree.size();i+=process_num) { // Gathered from 3/22 lecture
          cout << "i = " << i << endl;
          for (int j=i+1;j<genome_tree.size();j++) {
            ...
            z=compute_LCS(genome_tree[i].second,genome_tree[j].second);
              if (start) {
                start = false;
                // making buff was gathered from slides instead of doing max_i/max_j
                buff[0] = i;
                buff[1] = j;
                best = z;
            }
        ...
        string new_tree_label = "("+genome_tree[buff[0]].first + "," + genome_tree[buff[1]].first +")";
        genome_tree.erase(genome_tree.begin()+buff[0]);
        genome_tree.erase(genome_tree.begin()+buff[1]-1); // max_i got deleted!
  ...
  /**
   * lsof - list current files running/open
   * mpic++ [file] - compile
   * time mpiexec -np 4 a.out < covid5.dat - execute
   * lamhalt/lamwipe - to kill lamnodes
   */
  /**
   * Did covid-20 in 35 minutes
   */  